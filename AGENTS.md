...
## Значение команды пользователя «проверь»

Если пользователь пишет «проверь», это означает полный цикл: найти дефекты, исправить найденное, выполнить пропорциональную проверку, довести изменения до production и подтвердить production-ревизии и пользовательский flow. Исключение — пользователь явно ограничил задачу read-only проверкой или запретил изменения/deploy.

## Graphify: экономный поиск контекста в репозитории

Перед широким поиском, чтением множества файлов или исследованием зависимостей используй Graphify. Его задача — дать компактную карту кода и связи между символами, чтобы не тратить контекст на последовательные grep и чтение нерелевантных файлов.

### Установка

Если команда `graphify` недоступна:

1. Проверь, есть ли Python 3.10+ и `uv`.
2. Установи Graphify локально как инструмент:

   ```sh
   uv tool install "graphifyy[mcp]"
   ```

   Если `uv` недоступен, используй `pipx install "graphifyy[mcp]"`.

3. Зарегистрируй интеграцию для Codex:

   ```sh
   graphify codex install
   ```

Не изменяй глобальную конфигурацию, если проект или пользователь запрещает такие изменения. Перед установкой прочитай существующие `AGENTS.md`, `.codex/` и README.

### Построение и обновление графа

В корне репозитория построй граф:

```text
$graphify .
```

При последующих запусках обновляй только изменённые файлы:

```text
$graphify . --update
```

Не добавляй `graphify-out/` в Git и не коммить его без явного запроса пользователя. Сначала проверь размер, содержимое и правила репозитория.

### Как использовать

Используй Graphify прежде всего для вопросов о структуре и зависимостях:

- «Где определён и кем вызывается этот символ?»
- «Как запрос проходит от API до базы данных?»
- «Какие модули затронет изменение этого файла/метода?»
- «Какой путь связывает компонент A и компонент B?»
- «Какие основные подсистемы и точки входа есть в проекте?»

Предпочитай компактные запросы к графу, например:

```text
$graphify query "как запрос аутентификации попадает в базу?"
$graphify explain "AuthService"
$graphify path "ApiGateway" "DatabasePool"
```

После ответа графа открывай только указанные релевантные файлы и строки. Не редактируй код, опираясь только на граф: перед изменением прочитай фактическую реализацию, тесты и ближайшие вызовы.

### Правила качества

- Считай граф индексом, а не источником истины: он может устареть.
- После существенных изменений обнови граф через `--update`.
- Для поиска по документации, продуктовым требованиям и текстовым соглашениям используй обычный поиск или семантический поиск: граф лучше всего подходит для кода и связей.
- В финальном отчёте кратко укажи, использовался ли Graphify и какие файлы были выбраны через него.
