...
parser.add_argument(
    "--cleanup-plan", nargs=3, metavar=("OBSOLETE", "CURRENT", "ROLLBACK")
)
...
operation_count = sum(
    [
        ...
        bool(args.deploy),
        bool(args.cleanup_plan),
        bool(args.cleanup),
    ]
)
if operation_count != 1:
    parser.error("select exactly one operation")
if args.verify_boundary and not re.fullmatch(
    "[0-9a-f]{40}", args.expected_rollback_sha or ""
):
    parser.error("--expected-rollback-sha is required and must be a 40-character SHA")
for operation in (args.cleanup_plan, args.cleanup):
    if operation and not all(
        re.fullmatch("[0-9a-f]{40}", value) for value in operation[:3]
    ):
        parser.error("cleanup release arguments must be 40-character SHAs")
if args.cleanup:
    if not (args.recovery_archive and args.recovery_manifest):
        parser.error("--cleanup requires --recovery-archive and --recovery-manifest")
elif any((args.recovery_archive, args.recovery_manifest, args.prune_staged_copy)):
    parser.error("recovery arguments are valid only with --cleanup")

transport = importlib.import_module("ct137_ssh_transport")

if args.stage_file:
    try:
        path = args.stage_file.resolve(strict=True)
    except FileNotFoundError:
        parser.error("--stage-file does not exist")
    if not path.is_relative_to(ROOT):
        parser.error("--stage-file must be inside the canonical repository")
    if not re.fullmatch(
        r"(frontend-native-[0-9a-f]{40}\.(tar\.gz|manifest\.json)|cleanup-envelope-[0-9a-f]{64}\.json|kamilya-web-deploy(?:-[0-9a-f]{64})?\.py|ct137-web-(?:bootstrap|maintenance-update(?:-[0-9a-f]{64})?)\.sh)",
        path.name,
    ):
        parser.error("--stage-file has an unsupported name")
    if not re.fullmatch("[0-9a-f]{64}", args.expected_sha256 or ""):
        parser.error(
            "--expected-sha256 is required and must be 64 hexadecimal characters"
        )
    if transport.digest(path) != args.expected_sha256:
        parser.error("--stage-file digest does not match --expected-sha256")
    if path.suffix == ".gz":
        ...
        assert (
            ...
            manifest["libc"],
            manifest["api_url"],
        ) == ("linux", "x64", "musl", "https://api.kml.kz/api")
if args.deploy:
    sha, previous, digest = args.deploy
    if not (
        re.fullmatch("[0-9a-f]{40}", sha)
        and re.fullmatch("[0-9a-f]{40}", previous)
        and re.fullmatch("[0-9a-f]{64}", digest)
    ):
        parser.error(
            "--deploy requires two 40-character SHAs and one 64-character digest"
        )
...
def validate_recovery(
    archive: Path, manifest: Path, obsolete: str | None = None
) -> tuple[str, str]:
    archive = archive.resolve(strict=True)
    manifest = manifest.resolve(strict=True)
    if (
        archive.name != f"frontend-native-{archive.name[16:-7]}.tar.gz"
        or not re.fullmatch("[0-9a-f]{40}", archive.name[16:-7])
    ):
...
try:
    client.connect(
        transport.PROXY,
        username=values["PROXY_VPS_LOGIN"],
        password=values["PROXY_VPS_PASSWORD"],
        allow_agent=False,
        look_for_keys=False,
        timeout=15,
    )
...
"""
        ...
        remote_helper = (
            "/home/kamilya-admin/incoming/kamilya-web-deploy-" + helper_digest + ".py"
        )
        probe = (
            "import hashlib,pathlib,os,types,sys,unittest\nassert os.geteuid()!=0\nassert hashlib.sha256(pathlib.Path("
            + repr(remote_helper)
        ...
        for name in (
            "test_kamilya_web_deploy",
            "test_kamilya_web_cleanup",
            "test_kamilya_web_cleanup_controls",
        ):
            tests = (SOURCE_ROOT / ("scripts/ops/" + name + ".py")).read_text(
                encoding="utf-8"
            )
            tests = tests.replace(
                "ROOT = Path(__file__).resolve().parents[2]", "ROOT = pathlib.Path('/')"
            )
            tests = tests.replace(
                'ROOT / "infra/deploy/kamilya-web-deploy.py"',
                "Path(" + repr(remote_helper) + ")",
            )
            ...
            probe += (
                ...
                + repr(tests)
                + ',"ct137-helper-tests","exec"),module.__dict__)\nsuite.addTests(unittest.defaultTestLoader.loadTestsFromModule(module))\n'
            )
        probe += "result=unittest.TextTestRunner(verbosity=1).run(suite)\nraise SystemExit(not result.wasSuccessful())\n"
        stdin, out, err = client.exec_command(
            transport.PREFIX + shlex.quote("/usr/bin/python3 -I -"), timeout=90
        )
        stdin.write(probe)
        stdin.flush()
        stdin.channel.shutdown_write()
        output = out.read(4096).decode()
        errors = err.read(16384).decode()
        code = out.channel.recv_exit_status()
        print(output)
        print(errors)
        print(json.dumps({"native_test_exit": code}))
        if code:
        ...
        if args.prune_staged_copy:
            ...
            if (
                staged_code
                or staged_errors
                or staged_text not in ("STAGED_PAIR_PRESENT", "STAGED_PAIR_ABSENT")
            ):
                raise SystemExit("BLOCKED: staged cleanup preflight failed")
            staged_present = staged_text == "STAGED_PAIR_PRESENT"
        plan_command = (
            "/usr/bin/doas -n /usr/local/sbin/kamilya-web-deploy cleanup-plan "
            + " ".join((obsolete, current, rollback))
        )
        _, plan_out, plan_err = client.exec_command(
            transport.PREFIX + shlex.quote(plan_command), timeout=240
        )
        ...
        if (
            plan.get("release_sha"),
            plan.get("current_sha"),
            plan.get("rollback_sha"),
        ) != (obsolete, current, rollback):
        ...
        cleanup_command = (
            "/usr/bin/doas -n /usr/local/sbin/kamilya-web-deploy cleanup "
            + envelope_digest
        )
        _, cleanup_out, cleanup_err = client.exec_command(
            transport.PREFIX + shlex.quote(cleanup_command), timeout=240
        )
        ...
        staging_cleanup_ok = (
            not files_code and not files_errors and files_text == "USER_STAGING_CLEANED"
        )
        print(
            json.dumps(
                {
        ...
        if errors:
            try:
                ...
                if error_data.get("status") == "BLOCKED" and re.fullmatch(
                    "[a-z0-9_]+", error_data.get("reason", "")
                ):
