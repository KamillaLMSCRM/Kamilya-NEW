...
def _tenant_table(name: str) -> None:
    ...
    if name in {"course_approval_revisions", "course_approval_requests", "course_approval_reviewers", "course_review_attempts", "course_review_attempt_events"}:
        op.execute(f"REVOKE DELETE ON {name} FROM lms_app")
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        BEGIN
            IF TG_TABLE_NAME = 'course_approval_policies' THEN
                ...
                IF NEW.updated_by IS NOT NULL AND (SELECT tenant_id FROM users WHERE id = NEW.updated_by) IS DISTINCT FROM NEW.tenant_id THEN
            ...
            ELSIF TG_TABLE_NAME = 'course_approval_revisions' THEN
                SELECT tenant_id INTO owner_tenant FROM courses WHERE id = NEW.course_id;
                IF NEW.created_by IS NOT NULL AND (SELECT tenant_id FROM users WHERE id = NEW.created_by) IS DISTINCT FROM NEW.tenant_id THEN
            ...
            ELSIF TG_TABLE_NAME = 'course_approval_requests' THEN
                ...
                IF NEW.requested_by IS NOT NULL AND (SELECT tenant_id FROM users WHERE id = NEW.requested_by) IS DISTINCT FROM NEW.tenant_id THEN
    ...
    op.execute("""DO $$ BEGIN IF to_regclass('public.audit_logs') IS NOT NULL THEN REVOKE DELETE ON audit_logs FROM lms_app; END IF; END $$""")
...
def downgrade() -> None:
    op.execute("""DO $$ DECLARE table_name text; row_count bigint; BEGIN FOREACH table_name IN ARRAY ARRAY['course_approval_policies','course_approval_revisions','course_approval_requests','course_approval_reviewers','course_review_attempts','course_review_attempt_events','workflow_work_items','workflow_deliveries','workflow_access_credentials','workflow_reminders','workflow_escalations','workflow_idempotency_keys'] LOOP EXECUTE format('SELECT count(*) FROM %I', table_name) INTO row_count; IF row_count > 0 THEN RAISE EXCEPTION 'Refusing destructive course-approval downgrade: % rows remain in %', row_count, table_name USING ERRCODE='dependent_objects_still_exist'; END IF; END LOOP; END $$""")
    for name in ("course_approval_policies", "course_approval_revisions", "course_approval_requests", "course_approval_reviewers", "course_review_attempts", "course_review_attempt_events", "workflow_work_items", "workflow_deliveries", "workflow_access_credentials", "workflow_reminders", "workflow_escalations", "workflow_idempotency_keys"):
