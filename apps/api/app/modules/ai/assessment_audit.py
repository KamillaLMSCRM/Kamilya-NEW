...
def _coverage(verdict: Any, lessons: list[Any]) -> list[tuple[int, int]]:
...
Assess objectives using ONLY questions you keep IN THAT LESSON. An objective
covering several main subjects is uncovered when any main subject is missing.
There is NO minimum question count. Do not infer coverage merely from counts.
Return one decision per supplied ID and coverage for EVERY lesson:
{"decisions":[{"question_id":"L0Q0","decision":"keep","reason":"valid","duplicate_of":null}],
"coverage":[{"lesson_index":0,"uncovered_objective_indices":[1]}]}
Allowed: keep/valid/null OR drop/duplicate/distinct kept ID. No other decisions.
""", {
            "lessons": [{"lesson_index": li, "title": lesson.title, "objectives": lesson.objectives} for li, lesson in enumerate(lessons)],
        ...
        gaps = _coverage(verdict, lessons)
    filtered = []
    ...
    return AssessmentAuditResult(CourseAssessment(assessments=filtered), gaps, [decisions[qid] for qid in original])
