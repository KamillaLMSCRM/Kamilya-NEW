...
@pytest.mark.asyncio
async def test_resume_rejects_retired_generation_engine_before_queue_admission(monkeypatch):
    tenant_id = uuid4()
    job = SimpleNamespace(
        id="job-retired",
        tenant_id=tenant_id,
        status="interrupted",
        params={"source_analysis": {"analysis_mode": "direct_source"}},
    )
    monkeypatch.setattr(job_service, "_lock_tenant_row", AsyncMock())
    monkeypatch.setattr(job_service, "get_ai_job", AsyncMock(return_value=job))
    active_count = AsyncMock(side_effect=AssertionError("legacy job reached admission"))
    monkeypatch.setattr(job_service, "count_active_ai_jobs", active_count)

    with pytest.raises(job_service.AIJobSubmissionUnavailableError) as exc_info:
        await job_service.resume_interrupted_ai_job(
            _CommitSession(),
            job=job,
            tenant_id=tenant_id,
            task_kwargs={"job_id": "job-retired"},
            active_limit=2,
            worker_concurrency=2,
            historical_estimate_seconds=60,
            dispatcher=job_service.InMemoryAIJobDispatcher(),
    ...
    assert exc_info.value.detail == job_service.RETIRED_GENERATION_ENGINE_MESSAGE
    active_count.assert_not_awaited()
