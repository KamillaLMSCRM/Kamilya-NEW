    ...
    {confirm && confirmedRequest && <Modal open onClose={() => { if (!busyId) { setConfirm(null); setActionError(null); } }} title={confirm.kind === 'cancel' ? t('courseApproval.cancelConfirmTitle') : t('courseApproval.rotateCredentials')} description={confirm.kind === 'cancel' ? t('courseApproval.cancelConfirmDescription') : t('courseApproval.rotateConfirm')} dismissable={false} closeOnBackdrop={false}>
      <div className="space-y-4">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <Button variant="outline" onClick={() => { setConfirm(null); setActionError(null); }} disabled={busyId === confirm.requestId}>{t('courseApproval.cancel')}</Button>
          <Button onClick={() => void action(confirm.requestId, confirm.kind)} disabled={busyId === confirm.requestId}>{busyId === confirm.requestId ? t('courseApproval.sending') : confirm.kind === 'cancel' ? t('courseApproval.confirmCancel') : t('courseApproval.confirmRotate')}</Button>
        </div>
      </div>
    </Modal>}
    <Modal open={freshCredentials.length > 0} onClose={() => { setFreshCredentials([]); setCopiedKey(null); setCopyFailed(false); }} title={t('courseApproval.credentialsTitle')} description={t('courseApproval.credentialsWarning')} dismissable={false} closeOnBackdrop={false}>
      <section aria-label={t('courseApproval.credentialsTitle')} className="space-y-3 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm">
        ...
        {freshCredentials.map((credential) => <div key={credential.reviewer_id} className="space-y-2 rounded border bg-background p-3">
          <div className="flex flex-wrap items-center gap-2"><code className="min-w-0 flex-1 break-all text-xs">{credential.access_url}</code><Button size="sm" variant="outline" onClick={() => void copySecret(credential.access_url, `${credential.reviewer_id}:url`)}>{t('courseApproval.copyUrl')}</Button></div>
          <div className="flex flex-wrap items-center gap-2"><code className="break-all font-mono">{credential.temporary_pin}</code><Button size="sm" variant="outline" onClick={() => void copySecret(credential.temporary_pin, `${credential.reviewer_id}:pin`)}>{t('courseApproval.copyPin')}</Button></div>
        ...
        <Button className="w-full" onClick={() => { setFreshCredentials([]); setCopiedKey(null); setCopyFailed(false); }}>{t('courseApproval.credentialsAcknowledge')}</Button>
      </section>
    </Modal>
    <div className="space-y-3">
      {requests.map((request) => {
        ...
        return <Card key={request.request_id}><CardContent className="space-y-3 p-5">
          ...
          <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={busyId === request.request_id || effectiveOutcome !== 'pending'} onClick={() => requestConfirmation('cancel', request.request_id)}>{t('courseApproval.cancelRequest')}</Button><Button size="sm" variant="outline" disabled={busyId === request.request_id || !retryable} onClick={() => void action(request.request_id, 'resend')}>{t('courseApproval.retryDelivery')}</Button><Button size="sm" variant="outline" disabled={busyId === request.request_id || effectiveOutcome === 'cancelled'} onClick={() => requestConfirmation('rotate', request.request_id)}>{t('courseApproval.rotateCredentials')}</Button><Button size="sm" variant="outline" disabled={busyId === request.request_id || effectiveOutcome === 'cancelled'} onClick={() => void action(request.request_id, 'revoke')}>{t('courseApproval.revoke')}</Button></div>
        </CardContent></Card>;
      })}
    </div>
  </>;
