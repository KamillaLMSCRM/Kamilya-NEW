...
export function ApprovalRequestModal({ open, courseId, onClose, onCreated }: ApprovalRequestModalProps) {
  ...
  function updateGuestName(id: string, value: string) {
    setGuestRows((current) => current.map((row) => row.id === id ? { ...row, name: value } : row));
  }

  function updateGuestEmail(id: string, value: string) {
    setGuestRows((current) => current.map((row) => row.id === id ? { ...row, email: value } : row));
  }
  ...
  return <Modal open={open} onClose={dismissCredentials} title={created ? t('courseApproval.credentialsTitle') : t('courseApproval.send')} description={created ? t('courseApproval.credentialsWarning') : t('courseApproval.sendDescription')}>
    <div className="space-y-5">
      ...
      {!created && <><fieldset className="space-y-2"><legend className="text-sm font-medium">{t('courseApproval.reviewers')}</legend><div className="max-h-48 space-y-2 overflow-y-auto rounded-lg border p-3">{reviewers.length === 0 && <p className="text-sm text-muted-foreground">{t('courseApproval.noReviewers')}</p>}{reviewers.map((reviewer) => { const label = reviewer.full_name || reviewer.email || reviewer.id; return <label key={reviewer.id} className="flex min-h-10 items-center gap-3 text-sm"><input type="checkbox" checked={selected.includes(reviewer.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, reviewer.id] : current.filter((id) => id !== reviewer.id))} /><span className="min-w-0 break-words">{label}<span className="ml-2 text-xs text-muted-foreground">{reviewer.role === 'admin' ? t('courseApproval.administrator') : t('courseApproval.methodologist')}</span></span></label>; })}</div></fieldset><fieldset className="space-y-2 rounded-lg border p-3"><legend className="text-sm font-medium">{t('courseApproval.externalGuest')}</legend><div className="max-h-72 space-y-3 overflow-y-auto">{guestRows.map((row, index) => <div key={row.id} className="rounded border p-3"><p className="mb-2 text-xs font-medium text-muted-foreground">{t('courseApproval.externalGuest')} {index + 1}</p><div className="grid gap-2 sm:grid-cols-2"><Input aria-label={index === 0 ? t('courseApproval.guestName') : `${t('courseApproval.guestName')} ${index + 1}`} placeholder={t('courseApproval.guestName')} value={row.name} onChange={(event) => updateGuestName(row.id, event.target.value)} /><Input aria-label={index === 0 ? t('courseApproval.guestEmail') : `${t('courseApproval.guestEmail')} ${index + 1}`} placeholder={t('courseApproval.guestEmail')} type="email" value={row.email} onChange={(event) => updateGuestEmail(row.id, event.target.value)} /></div><Button type="button" size="sm" variant="outline" aria-label={`${t('courseApproval.removeGuest')} ${index + 1}`} onClick={() => removeGuestRow(row.id)}>{t('courseApproval.removeGuest')}</Button></div>)}</div><Button type="button" size="sm" variant="outline" onClick={addGuestRow} disabled={guestRows.length >= MAX_GUEST_REVIEWERS}>{t('courseApproval.addGuest')}</Button></fieldset><fieldset className="space-y-2"><legend className="text-sm font-medium">{t('courseApproval.delivery')}</legend><div className="grid gap-2 sm:grid-cols-2"><label className="flex min-h-11 items-center gap-2 rounded border p-3"><input type="radio" name="delivery" checked={deliveryMode === 'email'} onChange={() => setDeliveryMode('email')} /> {t('courseApproval.email')}</label><label className="flex min-h-11 items-center gap-2 rounded border p-3"><input type="radio" name="delivery" checked={deliveryMode === 'personal_link'} onChange={() => setDeliveryMode('personal_link')} /> {t('courseApproval.personalLink')}</label></div></fieldset><label className="block space-y-2 text-sm font-medium">{t('courseApproval.deadline')}<Input type="datetime-local" value={dueAt} onChange={(event) => setDueAt(event.target.value)} /></label><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={onClose} disabled={busy}>{t('courseApproval.cancel')}</Button><Button type="button" onClick={() => void submit()} disabled={busy}>{busy ? t('courseApproval.sending') : t('courseApproval.submit')}</Button></div></>}
    </div>
  </Modal>;
}
