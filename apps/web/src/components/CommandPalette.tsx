...
export default function CommandPalette() {
  ...
  const allItems: CommandItem[] = [
    ...
    { label: t('superadmin.tenants.title'), href: '/admin/super/tenants', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/></svg> },
    { label: t('providers.title'), href: '/admin/providers', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2 4 7v6c0 5 3.5 9.5 8 11 4.5-1.5 8-6 8-11V7l-8-5z"/><path d="m9 12 2 2 4-4"/></svg> },
  ];
  ...
  const canManageContent = user?.role === 'methodologist';
  const canManageAssignments = user?.role && ['methodologist'].includes(user.role);
  const canManageTenant = user?.role === 'admin' || user?.role === 'org_admin';
  ...
  if (canManageTenant) {
    allItems.push(
      { label: t('nav.userManagement'), href: '/admin/team', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
      ...
      { label: t('nav.kiosks'), href: '/admin/kiosks', icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="12" rx="2"/><path d="M8 20h8"/><path d="M12 16v4"/></svg> },
    );
  }
  ...
  const roleItems = allItems.filter((item) => item.href && canAccessRoute(user?.role, item.href));
