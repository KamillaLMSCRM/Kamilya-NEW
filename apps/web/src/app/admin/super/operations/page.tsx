...
type CeleryWorkerRole = {
  role: 'fast' | 'documents' | 'ai';
  status: WorkerHealth;
  reachable: boolean;
  worker_count: number;
  ...
  unexpected_queues: string[];
  unapproved_queue_count: number;
};
type OperationsSummary = {
  ...
  host: { cpu_percent: number | null; total_memory_bytes?: number | null; available_memory_bytes?: number | null; used_memory_bytes?: number | null; used_memory_percent?: number | null };
  filesystem: { total_bytes: number | null; free_bytes: number | null; used_percent: number | null };
  celery: { status: 'available' | 'unavailable'; health?: WorkerHealth; reachable: boolean; worker_count: number; workers?: CeleryWorkerRole[]; registered_required_tasks: string[]; missing_required_tasks: string[] };
...
export default function SuperadminOperationsPage() {
  ...
  const pressureTone = (value: number | null | undefined) => value === null || value === undefined ? 'border-slate-200 bg-slate-50' : value >= 90 ? 'border-red-200 bg-red-50' : value >= 80 ? 'border-amber-200 bg-amber-50' : 'border-emerald-200 bg-emerald-50';
  const workerTone = (status: WorkerHealth) => status === 'healthy' ? 'border-emerald-200 bg-emerald-50' : status === 'degraded' ? 'border-amber-200 bg-amber-50' : 'border-red-200 bg-red-50';
  const workerRoleLabels = {
    fast: t('superadmin.operations.celery.roles.fast'),
    documents: t('superadmin.operations.celery.roles.documents'),
    ai: t('superadmin.operations.celery.roles.ai'),
  };
  const workerHealthLabels = {
    healthy: t('superadmin.operations.celery.health.healthy'),
    degraded: t('superadmin.operations.celery.health.degraded'),
    unavailable: t('superadmin.operations.celery.health.unavailable'),
  };
  const effectiveCeleryHealth: WorkerHealth = summary?.celery.health ?? (summary?.celery.reachable ? 'degraded' : 'unavailable');
  const effectiveCeleryWorkers = summary?.celery.workers ?? [];
    ...
    <main className="mx-auto max-w-7xl space-y-6 p-6">
      ...
      {summary && <>
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4" aria-label={t('superadmin.operations.summary.title')}>
          ...
          <Card className={pressureTone(summary.host.used_memory_percent)}><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-lg"><Cpu className="h-5 w-5" />{t('superadmin.operations.host.title')}</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><Metric label={t('superadmin.operations.host.cpu')} value={formatPercent(summary.host.cpu_percent, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.host.memoryUsed')} value={formatPercent(summary.host.used_memory_percent, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.host.memoryTotal')} value={formatBytes(summary.host.total_memory_bytes, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.host.memoryAvailable')} value={formatBytes(summary.host.available_memory_bytes, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.host.memoryUsedBytes')} value={formatBytes(summary.host.used_memory_bytes, t('superadmin.operations.unavailable'))} /></CardContent></Card>
          <Card className={pressureTone(summary.filesystem.used_percent)}><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-lg"><HardDrive className="h-5 w-5" />{t('superadmin.operations.filesystem.title')}</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><Metric label={t('superadmin.operations.filesystem.used')} value={formatPercent(summary.filesystem.used_percent, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.filesystem.total')} value={formatBytes(summary.filesystem.total_bytes, t('superadmin.operations.unavailable'))} /><Metric label={t('superadmin.operations.filesystem.free')} value={formatBytes(summary.filesystem.free_bytes, t('superadmin.operations.unavailable'))} /></CardContent></Card>
          <Card className={`md:col-span-2 ${workerTone(effectiveCeleryHealth)}`}><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-lg"><Server className="h-5 w-5" />{t('superadmin.operations.celery.title')}</CardTitle></CardHeader><CardContent className="space-y-4 text-sm"><Metric label={t('superadmin.operations.celery.status')} value={workerHealthLabels[effectiveCeleryHealth]} danger={effectiveCeleryHealth === 'unavailable'} /><Metric label={t('superadmin.operations.celery.workers')} value={summary.celery.worker_count} />{effectiveCeleryWorkers.length > 0 ? <ul className="grid gap-3 sm:grid-cols-3" aria-label={t('superadmin.operations.celery.rolesTitle')}>{effectiveCeleryWorkers.map((worker) => <li key={worker.role} className={`min-w-0 space-y-2 rounded-md border p-3 ${workerTone(worker.status)}`}><div className="flex items-start justify-between gap-2"><strong>{workerRoleLabels[worker.role]}</strong><span className="text-xs font-medium">{workerHealthLabels[worker.status]}</span></div><Metric label={t('superadmin.operations.celery.expectedQueues')} value={worker.expected_queues.join(', ')} /><Metric label={t('superadmin.operations.celery.activeQueues')} value={worker.active_queues.join(', ') || t('superadmin.operations.none')} />{worker.missing_queues.length > 0 && <TaskList label={t('superadmin.operations.celery.missingQueues')} tasks={worker.missing_queues} empty={t('superadmin.operations.none')} danger />}{worker.unexpected_queues.length > 0 && <TaskList label={t('superadmin.operations.celery.unexpectedQueues')} tasks={worker.unexpected_queues} empty={t('superadmin.operations.none')} danger />}{worker.unapproved_queue_count > 0 && <Metric label={t('superadmin.operations.celery.unapprovedQueues')} value={worker.unapproved_queue_count} danger />}</li>)}</ul> : <p className="rounded-md border border-dashed px-3 py-3 text-muted-foreground">{t('superadmin.operations.unavailable')}</p>}<TaskList label={t('superadmin.operations.celery.registered')} tasks={summary.celery.registered_required_tasks} empty={t('superadmin.operations.none')} /><TaskList label={t('superadmin.operations.celery.missing')} tasks={summary.celery.missing_required_tasks} empty={t('superadmin.operations.none')} danger={summary.celery.missing_required_tasks.length > 0} /></CardContent></Card>
