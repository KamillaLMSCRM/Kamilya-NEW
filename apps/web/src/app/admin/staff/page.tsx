...
export default function AdminStaffPage() {
  ...
  const [savedMappings, setSavedMappings] = useState<Array<{ id: string; name: string; is_default: boolean }>>([]);
  ...
  const handleSaveCurrentMapping = async () => {
    ...
    try {
      ...
      setSavedMappings((cur) => [{ id: res.data.id, name: res.data.name, is_default: res.data.is_default }, ...cur]);
    } catch (err: any) {
          ...
          {tab === "import" && (
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 border-t border-border pt-4">
                <Button type="button" variant="outline" onClick={() => setLegacyImportOpen((value) => !value)}>
                  {legacyImportOpen ? "Скрыть старый импорт" : "Открыть старый импорт"}
                </Button>
                <span className="text-xs text-muted-foreground">
                  Запасной сценарий для прежних файлов и совместимых интеграций
                </span>
              </div>
         ...
         /commit если бэк вернул task_id. Polling внутри компонента. */}
              ...
              <Card>
                ...
                <CardContent className="text-sm text-foreground space-y-2">
                  <div>
                    Загрузите Excel <strong>.xls</strong>, <strong>.xlsx</strong> или CSV-файл. Старые штатные
                    ведомости могут содержать отдельные строки с адресами филиалов — система создаст по ним отделы.
                    Для CSV подходят кодировки UTF-8 и Windows-1251.
                  </div>
                  <div>
                    <strong>Обязательные данные:</strong> табельный номер, ФИО (или отдельные имя и фамилия),
                    должность и отдел. В старой филиальной штатке отдел определяется по строке с городом.
                  </div>
                  <div>
                    <strong>Дополнительно:</strong> email, телефон, дата приёма.
                  </div>
                  <div className="text-muted-foreground text-xs">
                    Заголовки могут находиться не в первой строке. Можно писать по-русски или латиницей: Сотрудник /
                    ФИО / full_name, Табельный номер / personnel_number, Имя / first_name, Фамилия / last_name,
              ...
              The default mapping is auto-applied on file upload. */}
                  {savedMappings.length > 0 && (
                    <div className="flex flex-wrap items-center gap-3 rounded-md border border-border bg-muted/30 px-3 py-2">
                      <label className="text-sm font-medium text-foreground">Шаблон колонок:</label>
                      <select
                  ...
                  {savedMappings.length === 0 && Object.keys(columnMapping).length > 0 && (
                    <div className="text-xs text-muted-foreground">
                      <Button type="button" variant="ghost" size="sm" onClick={handleSaveCurrentMapping}>
                          ...
                          <div className="overflow-x-auto rounded-xl border border-border">
                            <Table>
                              <thead className="bg-muted">
                                <tr>
                                  {(preview.raw_columns || []).map((column) => (
                                    <th key={column} className="p-2 text-left text-xs whitespace-nowrap">
                                      {column}
                                    </th>
                                  ))}
                                </tr>
                                ...
                                {(preview.sample_rows || []).map((row, idx) => (
                                  <tr key={idx} className="border-t">
                                    {(preview.raw_columns || []).map((column) => (
                                      <td
                                        key={`${idx}-${column}`}
                                        className="p-2 text-xs text-muted-foreground whitespace-nowrap"
                                      >
                                        {row[column] || "—"}
                                      </td>
                                    ))}
                                ...
                                <tbody>
                                  {preview.items.map((item) => (
                                    <tr key={item.row_number} className="border-t">
                                      <td className="p-2">
                                        <span
                                          className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${ACTION_COLORS[item.action]}`}
                                        >
                                ...
                                <tbody>
                                  {preview.invalid_rows.map((inv, i) => (
                                    <tr key={i} className="border-t">
                                      ...
                                      <td className="p-2 text-xs text-destructive">{inv.errors.join(", ")}</td>
