...
export default function InvitationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('invitations.loadError'), { description: error?.response?.data?.detail });
  ...
  const createInvitation = async () => {
    ...
    } catch (error: any) { toast.error(t('invitations.createError'), { description: error?.response?.data?.detail }); }
  ...
  const resend = async (id: string) => {
    ...
    catch (error: any) { toast.error(t('invitations.resendError'), { description: error?.response?.data?.detail }); }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <Card><CardHeader><CardTitle>{t('invitations.createTitle')}</CardTitle></CardHeader><CardContent className="flex flex-col gap-3 sm:flex-row">
