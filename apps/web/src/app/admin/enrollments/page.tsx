...
export default function EnrollmentsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [coursesRes, usersRes] = await Promise.all([
        ...
        // P0-2 fix: include_students=true (admin-only) — otherwise the
        // 200 employees imported via /admin/staff are silently filtered.
        // per_page=200 fits ~300 employees per tenant without paging.
        fetch(
          `${API_URL}/v1/users?per_page=200&include_students=true`,
          { headers: { Authorization: `Bearer ${token}` } },
        ),
      ]);
      if (coursesRes.ok) {
  ...
  const availableUsers = useMemo(() => {
    return users.filter(
      (u) => matchesUserQuery(u, userSearch) && !enrolledUserIds.has(u.id),
    );
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">
        {t('nav.userManagement')} — {t('courses.enrollments')}
      </h1>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardContent className="p-4 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              {selectedCourse
                ? `Доступно: ${availableUsers.length} из ${users.length}`
                : 'Сначала выберите курс слева'}
            </p>
            <SearchInput
              value={userSearch}
              onChange={setUserSearch}
              placeholder="Найти сотрудника по имени, email или табельному…"
            />
