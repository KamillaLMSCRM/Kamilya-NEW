...
export default function CohortsPage() {
  ...
  useEffect(() => { load(); }, [load]);
  const open = async (item: Cohort) => { setSelected(item); setName(item.name); setDescription(item.description); /* links are replaced from the current selection */ };
  const create = async () => { if (!name.trim()) return; setSaving(true); try { const response = await api.post<Cohort>('/v1/cohorts', { name: name.trim(), description }); setItems((list) => [response.data, ...list]); setSelected(response.data); toast.success(t('cohorts.created')); } catch (error: any) { toast.error(t('cohorts.saveFailed'), { description: error?.response?.data?.detail || error?.message }); } finally { setSaving(false); } };
