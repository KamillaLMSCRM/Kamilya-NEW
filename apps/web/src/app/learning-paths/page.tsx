...
export default function LearningPathsPage() {
  ...
  if (isLearner) return <LearnerView programs={learnerPrograms} t={t} />;
            ...
            {paths.map((path) => (
              ...
              >
                ...
                <div className="mt-2 flex flex-wrap gap-1.5 text-xs text-muted-foreground">
                  ...
                  <span>{path.course_count} {t('learningPaths.courses')}</span>
                  <span>{path.assignment_count == null ? '—' : path.assignment_count} {t('learningPaths.learners')}</span>
                </div>
              </button>
            ))}
          </div>
          ...
          <section className="min-w-0 rounded-lg border bg-background p-4 sm:p-6" aria-label={t('learningPaths.editor')}>
            <div className="mb-6 flex flex-col gap-3 border-b pb-5 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                {selected && <p className="mt-1 text-sm text-muted-foreground">v{selected.version || 1} · {selected.course_count} {t('learningPaths.courses')} · {selected.assignment_count == null ? '—' : selected.assignment_count} {t('learningPaths.learners')}</p>}
              </div>
