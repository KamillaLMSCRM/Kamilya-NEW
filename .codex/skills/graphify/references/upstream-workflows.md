# Archived upstream Graphify workflows — non-operative

This file preserves upstream reference material only. It is not a skill,
runbook, project policy, or executable fallback. Its POSIX commands, package
installation, hooks/watchers, remote cloning, uploads, database pushes, and
destructive examples are not approved for this Windows workspace. Always return
to `../SKILL.md`, verify the installed `graphify 0.9.23` CLI, and use only the
bounded Kamilya procedure selected there.

# Historical `/graphify` reference
