...
**Критическая ошибка:** у меня уже есть прямой доступ к живому production:
- `DATABASE_URL` в `apps/api/.env` (Supabase pooler)
- `RENDER_API_KEY` в `apps/api/.env` (deploy state + CLI для runtime logs)
- `https://kamilya-lms-api.onrender.com` — живой, health 200

Askar указал на это в начале хода. **Поднимать локально не нужно было.**
...
- Live QA: используй production credentials вместо docker
