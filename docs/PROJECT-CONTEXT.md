...
- Отдельный публичный лендинг использует только собственный
  `C:\Kamilya New\kamilya-landing\.env.local`: `github_landing_token` для
  `KamillaLMSCRM/kamilya-landing` и `vercel_landing_token` для связанного
  Vercel project. Не подменять их credentials из `Kamilya-NEW`; перед provider
  mutation проверять только наличие точного имени без вывода значения.
