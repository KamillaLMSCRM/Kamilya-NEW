...
запрашивает шестизначный OTP, который хранится отдельно от обычного login OTP и
привязан к `invitation_id`. Resend доставляет код на email, указанный
методологом. Backend проверяет tenant, срок, статус и OTP, после чего активирует
существующий `User`, фиксирует `email_verified_at`,
