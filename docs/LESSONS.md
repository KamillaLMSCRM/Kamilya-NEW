...
## 2026-07-24 — Self-service role assignment must refresh session UI state

### Наблюдение

Успешный `POST /users/{id}/roles` обновлял таблицу команды, но при назначении
роли самому себе `authStore.user.roles` оставался прежним. Переключатель рабочего
режима в TopBar поэтому появлялся только после обновления сессии.

### Detection rule

Если mutation меняет права текущего пользователя, проверять не только ответ API
и целевую таблицу, но и session-derived UI в том же браузере без reload/logout.
Успешный ответ должен синхронно обновить локальное auth-состояние, сохраняя
текущую активную роль до явного переключения пользователя.
