...
Профиль должности и обязательное обучение редактируются в `/positions`, группы
обучения — в `/cohorts`, фактические назначения — в `/assignments`. Экран
структуры не дублирует эти редакторы.

### 6.2.1 Каноническая карточка квалификационных требований должности

Канонический экран методолога для настройки должности: `/positions/{position_id}`. Реестр `/positions` используется для поиска, создания и открытия карточки, но не является вторым местом редактирования требований. Карточка объединяет сведения, которые раньше были распределены между должностями, матрицей компетенций, правилами обучения и onboarding-настройками.
...
для опубликованного курса и ведёт на `/assignments`; окончательное действие
выполняет методолог на каноническом экране.
