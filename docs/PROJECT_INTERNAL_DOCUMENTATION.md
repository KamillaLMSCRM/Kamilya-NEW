...
Worker скачивает сохранённый исходник, конвертирует его через локальный Docling,
перестраивает pgvector index и переводит документ в `ready`, `partial` или
`failed`. Бинарный файл нельзя заменять текстовой заглушкой: недоступность
Docling является терминальной ошибкой индексации. Старый Word `.doc`
предварительно конвертируется в `.docx` headless LibreOffice. Базовый лимит
