...
- черновик доступен методологу/преподавателю, но не обучающемуся;
...
python -m pytest apps/api/tests/test_methodologist_role.py apps/api/tests/test_role_boundaries.py apps/api/tests/test_kiosk_jwt.py -q
cd apps/web
npm run typecheck
npx next build
```
