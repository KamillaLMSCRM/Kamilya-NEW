...
На период первичного рекламного запуска API после commit дополнительно
отправляет полную операторскую копию публичной заявки через Resend на адрес из
`PUBLIC_LEAD_NOTIFICATION_EMAIL`. Письмо включает контактные поля, параметры
формы, UTM/GCLID, ROI-контекст, версию согласия, серверное время приёма и ID
заявки. HTML-значения экранируются; provider idempotency key строится из ID
заявки. Это вторичный best-effort канал: его ошибка не меняет успешный ответ
формы, поскольку источниками истины остаются `TenantLead` и durable CRM outbox.
Google Sheets в этом временном контуре не используется. После появления
принятого операторского интерфейса CRM пересылка отключается пустым значением
...
- `/courses/templates/{blueprint_id}`: инстанцирование и адаптация
  версионированной отраслевой заготовки;
- `/courses/[id]/edit`: редактирование модулей и уроков.

Отраслевая заготовка не вводит второй lifecycle курса. Каталог является
tenant-agnostic и версионируется в коде; `POST /v1/course-blueprints/{id}/instantiate`
создаёт обычный native draft. Происхождение, версия, локаль, ответы checklist,
выбранные tenant-документы и render hashes хранятся в `Course.source_analysis`.
`GET/PUT /v1/courses/{course_id}/blueprint-adaptation` работает только с курсом
текущего tenant. Блокировка строки tenant сериализует проверку дубля: одна
активная версия одной локали создаётся только один раз.

Первая заготовка `kz-finance-information-security` версии `2026.1` содержит
один модуль, восемь непересекающихся уроков, отдельный quiz каждого урока и
шестнадцать single-choice вопросов. Маркетинговая пропорция 70/30 хранится как
явная оценка. Для 100% готовности обязательны восемь tenant-specific ответов.
Выбранные документы используются как источники методологической проверки и не
подмешиваются в текст автоматически.

Обновление адаптации разрешено только для draft и только пока сохранённые
структура и SHA-256 текста совпадают с последним render state. Это не позволяет
перезаписать ручную работу методолога. После изменения ответы снова переводят
курс в `review_status=pending`; review endpoint не одобряет blueprint-курс до
100% checklist. Дальше используются существующие publish, ContentRelease,
assignment и evidence-механизмы.
...
7. Embeddings используют совместимые Voyage V4
   (`voyage-4-lite -> voyage-4 -> voyage-4-large`), затем три приватные реплики
   Qwen3 (`gx10-12 -> gx10-2 -> gx10-4`) и Cohere. Модели Voyage V4 имеют общий
   канонический embedding-space identity; реплики Qwen также имеют отдельный общий
   identity. Векторы разных семейств никогда не сравниваются. Поисковые запросы
   одного курса отправляются batch-вызовом, чтобы не исчерпывать RPM. После
   добавления способа оплаты production-shaped запрос из 250 фрагментов и
   22 549 токенов прошёл через `voyage-4-large` одним HTTP 200; полный Excel из
   1 076 чанков прошёл через основной `voyage-4-lite` за 7,725 секунды. Если все
   провайдеры временно недоступны,
   синтетические векторы не создаются: документ остаётся частично готовым и
   доступным для генерации по проверенному оригиналу, а семантический поиск явно
   помечается как недоступный.

   Каждый embedding progress event содержит активного provider и номер попытки.
   При failover счётчик конкретного provider начинается с нуля, но интерфейс
   показывает `#2`, `#3` и далее; это смена полной попытки, а не потеря уже
   записанных данных. Провайдерное имя проходит закрытую безопасную проекцию API.
...
`reuse_reason` запрос останавливается до trial/LLM charging и возвращает
проекцию `id/title/status`. После явного выбора причины создаётся новый draft;
причина проходит через `AIJob.params`, Celery kwargs и сохраняется в
`Course.source_analysis`. Регенерация существующего draft с `course_id` не
является созданием второго курса и не попадает в этот gate.

Точная копия загружаемого файла определяется по SHA-256. Быстрый API-check
дополнен частичным уникальным индексом PostgreSQL по
`(tenant_id, content_sha256)` для активных документов. Поэтому два
конкурентных запроса не создают две активные записи; проигравший запрос не
записывает объект в storage и получает тот же `duplicate_document`.
...
Создание курса помечает факт reservation в `AIJob.params`; единый idempotent
cleanup освобождает trial quota и AI budget только один раз. Если cleanup после
фиксации `cancelled` временно не прошёл, повторная отмена снова выполняет cleanup
и возвращает успешный ответ только после commit освобождения.
...
Действующий production-путь генерации один: `evidence_v2`. Pipeline вызывает
`evidence_engine.application.generate_evidence_course`, который строит
неизменяемый план фактов, адаптивно формирует уроки, реализует их по тому же
набору evidence и создаёт вопросы через semantic assessment axes. Старые
`assessment.py`, `assessment_audit.py` и `assessment_completion.py` удалены;
возврат их импортов блокирует исполняемый архитектурный тест. Общими остаются
только отдельные source/quality-модули, которыми реально пользуется V2.

Semantic assessment не добивает тест до заранее заданной квоты. Для одного
урока выбирается не более трёх доказуемых осей; слабая, неоднозначная или
неподтверждённая формулировка удаляется либо проходит bounded repair. В
`assessment_review` сохраняются `block_outcomes`, `axis_outcomes`, причины
пропуска и coverage. Локальный `analyze_evidence_v2_replay.py` агрегирует эти
данные и показывает уроки, где доступных осей больше трёх, не вызывая модель и
не меняя БД.

Для структурированных документов Evidence V2 использует worksheet/section
границы, document passport и source locators, а не свободный план отдельного
Architect. Один source fact назначается одному владельцу в плане курса; соседние
разделы и вспомогательные листы не становятся темами только ради числа уроков.
Для OCR и plain text границы восстанавливаются из проверенных заголовков и
контiguous source blocks, сохраняя ссылку каждого утверждения на оригинал.
...
Детерминированный source-only fallback при исчерпании генеративных providers
никогда не получает `verified`: каждый затронутый урок сохраняется как
`needs_review`, а курс содержит закрытую degraded-диагностику. Исходные Markdown
и HTML-маркеры нейтрализуются до сохранения fallback-текста и отображаются
безопасным renderer как обычный текст.
...
`/assignments` является видимым каноническим экраном ручной mutation в разделе
назначения обучения и входит в sidebar и command palette. Контекстные точки входа
из курса и сотрудника передают `course_id` или `user_id`, а UI предвыбирает
известный контекст. Назначаются только опубликованные курсы.

Навигация методолога строится из единого `routeRegistry` по этапам работы:
`Документы → AI Генерация → Курсы → Конструктор тестов`, затем программы и
назначения, затем `Сотрудники и структура → Группы сотрудников → Тестирование
кандидатов`, затем журнал, подтверждение и сроки хранения. `/staff` является
канонической ссылкой меню; query-параметр `tab` сохраняет вкладки импорта и
структуры, не снимая активное состояние пункта. Cohort остаётся сохранённой
аудиторией и не становится подразделением или владельцем курсов.
...
Каждый authenticated request повторно проверяет credential, поэтому отзыв,
отмена назначения или блокировка сотрудника немедленно закрывают уже открытый
сеанс. Срок публичной ссылки ограничивает только новый обмен PIN: после
успешного первого входа сессия живёт по собственному JWT и access policy.
Перевыпуск до первого входа отзывает старую credential; после старта окна он
запрещён, а продление/отзыв требуют отдельной причины и audit event. Kiosk и
общий табельный номер в этом flow не используются.

`EnrollmentAccessPolicy` хранит выбранный способ доставки, срок ссылки,
необязательный абсолютный дедлайн и окно прохождения. Окно стартует один раз
при первом успешном обмене PIN. Assignment-access JWT содержит точный
`enrollment_id`; чтение курса, progress, quiz и completion проверяют именно
этот Enrollment и не подменяют его другой recurring/manual записью того же
курса. Student-safe status endpoint отдаёт только server time и политику для
обратного отсчёта, без token, PIN или PII.
...
- Для должностной инструкции сохраняется ограничение размера черновика. Число
  вопросов, как и для обычного курса, определяется доказуемыми assessment axes:
  не более трёх на урок и без дополнения слабыми вопросами до квоты.
...
Сотрудник может скачать сокращённый PDF собственного исходного события через
learner-owned export. Он доступен до отдельного email OTP и не содержит
правильных ответов, служебного audit payload или данных других пользователей.
Полученный головным офисом подписанный PDF/JPEG/PNG методолог добавляет в
append-only `training_evidence_signed_scans`. Tenant/user/enrollment выводятся
из исходного события; исходный результат, попытка и сертификат не изменяются.
...
0102 автоматическое соблюдение срока хранения данных кандидатов
0103 отдельные enrollment instance для повторных циклов обучения
0104 неизменяемые доказательства принятия legal-документов при регистрации
0105 tenant-scoped защита от активных дублей исходных документов
