...
конвертер на VPS. PDF и изображения сначала обрабатываются Docling, потому что
для них важны OCR, структура страниц и таблиц. DOCX/XLS/XLSX сначала проходят
через MarkItDown; при непригодном результате используется Docling. Старый Word
`.doc` предварительно преобразуется headless LibreOffice в `.docx`, после чего
идёт по тому же Office-маршруту. MarkItDown работает без plugins и принимает
только локальный временный файл, без URL и внешнего OCR.

Результат задания сохраняет движок, его версию, факт fallback и предупреждения.
Для PDF fallback на MarkItDown помечается как деградированный, поскольку он не
заменяет OCR и точное восстановление layout. Если оба допустимых движка не дали
...
источника 50 МБ и может быть изменён через `MAX_DOCUMENT_SIZE_MB`.
