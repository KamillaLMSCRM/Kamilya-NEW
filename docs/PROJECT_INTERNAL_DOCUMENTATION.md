...
7. Embeddings используют управляемую цепочку трёх приватных реплик Qwen3
   (`gx10-12 -> gx10-2 -> gx10-4`), затем Voyage и Cohere. Маршруты имеют разные
   диагностические имена и фактические API model ID, но один канонический
   embedding-space identity, поэтому failover между репликами не ломает поиск по
   уже сохранённым векторам. Если все провайдеры временно недоступны,
