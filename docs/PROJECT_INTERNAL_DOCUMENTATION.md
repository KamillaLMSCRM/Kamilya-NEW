...
Для структурированных документов Architect использует нумерованные разделы
как исходные границы уроков и назначает каждой теме одного владельца во всём
курсе. Writer получает названия уроков из всех модулей, а не только текущего,
и не должен создавать итоговый урок-пересказ. При OCR lexical fallback точное
совпадение с `relevant_headings` имеет приоритет над длиной фрагмента и общими
терминами; это ограничивает попадание соседних разделов в урок.
