...
`org_admin`) and `methodologist` (and `teacher`) — but their
...
|---------------------|---------------------------------------------------------------|
| **System user**     | Anyone who logs in to manage the LMS: `admin`, `org_admin`, `methodologist`, `teacher`, `superadmin`. Carries an `access_token`. Distinct from "обучающиеся" below. |
...
| **Methodologist**   | `methodologist` / `teacher` — owns the *learning content and staff configuration*: courses, JDs, position/department bindings, quiz authoring, onboarding quiz reset, staff import (Excel/CSV), course review workflow. |
...
- `methodologist` = instructional designer / L&D manager (curriculum
  decisions).
- `teacher` = course instructor / subject-matter expert (content
  edits, attempts grading).

Where the spec says "methodologist", we mean **either** of these.
...
│ • system user list (admin / methodologist / teacher invite / off)  │
...
└────────────────────────────────────────────────────────────────────┘
┌────────────────────────────────────────────────────────────────────┐
│ Methodologist (teacher) — "методолог обучения"                     │
...
|---------------------------|---------------------|--------------------------------------------------------|
| `/admin/users` (legacy)   | admin               | list of admin/methodologist/teacher accounts           |
| `/admin/team` (ADR-0011)  | admin               | same, renamed                                          |
| `/admin/staff`            | methodologist       | Импорт (Stage 1d) + Структура + Правила (B2)            |
| `/admin/enrollments`      | admin + methodologist | directly enroll a user — escape hatch (revised 2026-06-30, see §6) |
...
| `GET /v1/admin/staff/structure`                           | `admin`, `org_admin`, `superadmin`, `methodologist`, `teacher` | existing |
...
| `POST /v1/courses/{id}/enrollments` (bulk by manager)     | `admin`, `org_admin`, `superadmin`, `teacher` (methodologist) | revised 2026-06-30 — direct user→course assignment is content-domain, not tenant-infra (§6) |
| `GET /v1/courses/{id}/enrollments` (list)                 | `admin`, `org_admin`, `superadmin`, `teacher` | same — content-domain, was open to all auth'd (added role guard) |
| `DELETE /v1/courses/enrollments/{id}`                     | `admin`, `org_admin`, `superadmin`, `teacher` | same — was open to all auth'd (added role guard) |
| `GET /v1/courses/{id}/enrollment-stats`                   | `admin`, `org_admin`, `superadmin`, `teacher` | already shared |
| `GET /v1/admin/export/enrollments` (CSV)                  | `admin`, `org_admin`, `superadmin`, `teacher` | revised 2026-06-30 — methodologist needs CSV for HR handoff |
...
# Methodologist CANNOT:
# - GET /v1/users (without ?include_students=true; that flag rejects them)
# - POST /v1/team/invitations
# - POST /v1/courses/{id}/enrollments (now allowed for methodologist — see §6)
# - POST /v1/kiosks/*
# - GET /v1/admin/super/*
```

## 6. Revision 2026-06-30 — enrollments joined to content domain

**Decision:** direct user→course assignment (`/admin/enrollments`,
`POST/GET/DELETE /v1/courses/{id}/enrollments`,
`GET /v1/courses/{id}/enrollment-stats`,
`GET /v1/admin/export/enrollments`) is now allowed for **both admin
and methodologist (teacher)**, not admin-only as originally written in §3.

**Why:** the §3 table tagged `POST /v1/courses/{id}/enrollments` as
"admin-only by design", but that contradicted the
`TZ_COURSE_ASSIGNMENT_ACCESS_v1.md` model:

- Level 1 (position→course) and Level 2 (department→course) are
  automated via `recompute_enrollments` kernel.
- Level 3 (auto-enrollment rules) is also kernel-managed.
- **Level 4 ("manual overrides")** is the escape hatch — when a
  course needs to be assigned to one specific person that the
  position/department rules don't cover (e.g. a contractor, an
  employee in transition, a one-off compliance nudge).

Level 4 is content work, not tenant infrastructure. Methodologists
who own course authoring and the position/department hierarchy
also need to make these one-off assignments. Forcing them to ask
admin for every override creates a bottleneck for what is otherwise
a single-click UI action.

**Methodologist scoping on `/admin/enrollments`:**

- Backend: `require_role("superadmin", "admin", "org_admin", "teacher")`
  on every enrollment mutate/list/stats/export endpoint.
  Was: `get_current_user` only on GET/DELETE — a latent bug that
  let any tenant user (including students) read and delete
  enrollments. Fixed in the same revision.
- Frontend: `/admin/enrollments` already had `teacher` in the
  Sidebar `roles:` array (added 2026-06-27). CommandPalette was
  hardcoded to `['admin', 'superadmin']` and excluded methodologist
  from the Cmd-K shortcut — fixed to also include `org_admin` and
  `teacher`. Quizzes moved with enrollments since they share the
  same content-domain grouping.
- User picker: backend `GET /v1/users?include_students=true` stays
  admin-only (admin needs full roster; methodologist should not
  see plain student accounts). The `/admin/enrollments` page reads
  users without that flag by default, so methodologists see the
  staff roster only.

**Not changed:** admin still owns tenant infrastructure
(`/admin/kiosks`, `/admin/settings`, `/admin/integrations`,
`/admin/audit-log`, `/admin/super/*`).

Add this test as part of this epic, alongside the B2 UI work, so
the separation is enforced by CI before any UI lands.
...
## Action items (revision 2026-06-30, §6)

1. ✅ Add `teacher` to `require_role(...)` on `POST/GET/DELETE /v1/courses/{id}/enrollments` and `GET /v1/admin/export/enrollments`.
2. ✅ Add `teacher` (and `org_admin`) to `CommandPalette.tsx` so Cmd-K shows Enrollments/Quizzes for methodologists.
3. ✅ Drop misleading "admin-only" comment from `Sidebar.tsx` (item already had teacher in `roles:`).
4. 🔲 Add regression test: methodologist can hit `POST /v1/courses/{id}/enrollments`, student gets 403.
5. 🔲 Add regression test: methodologist can hit `GET /v1/admin/export/enrollments`, student gets 403.
