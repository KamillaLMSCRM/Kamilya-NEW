...
Direct learner-course assignment is **methodologist/teacher-owned**, not
...
`methodologist`/`teacher` and reject tenant `admin`/`org_admin` by default.
...
| `POST /v1/courses/{id}/enrollments` (bulk by manager)     | `methodologist`, `teacher` | revised 2026-07-01 - direct learner-course assignment is content-domain, not tenant-infra (§6) |
| `GET /v1/courses/{id}/enrollments` (list)                 | `methodologist`, `teacher` | content-domain assignment list |
| `DELETE /v1/courses/enrollments/{id}`                     | `methodologist`, `teacher` | only manual enrollments may be removed directly |
| `GET /v1/courses/{id}/enrollment-stats`                   | `admin`, `org_admin`, `superadmin`, `teacher` | already shared |
| `GET /v1/admin/export/enrollments` (CSV)                  | `methodologist`, `teacher` | assignment export for learning-content handoff |
...
# - POST /v1/courses/{id}/enrollments (allowed for methodologist/teacher; denied for tenant admin)
# - POST /v1/kiosks/*
# - GET /v1/admin/super/*
```
...
**Decision:** direct learner-course assignment is methodologist/teacher-owned
...
- Sidebar and CommandPalette show assignments only to `methodologist` / `teacher`.
- Backend guards for bulk assignment/list/delete/export allow `methodologist` / `teacher`.
...
3. ✅ Restrict assignment navigation to `methodologist` / `teacher`.
4. ✅ Restrict backend assignment/list/delete/export guards to `methodologist` / `teacher`.
