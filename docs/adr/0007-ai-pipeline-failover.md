...
2. `custom:qwen38-flash-next` — `qwen3.8-flash-next` на ASUS;
3. `glm53-flash-asus` — `LibertAIDAI/GLM-5.3-Flash-NVFP4` на ASUS.

Цепочка является единственным пользовательским маршрутом генерации. Старый
Qwen не входит в неё и не возвращается при недоступности нового endpoint-а.
...
`10.66.66.28:8000`. Поэтому Qwen и GLM пока не считаются рабочими production
fallback до исправления маршрута и отдельного успешного VM126 readback.
DeepSeek остаётся первым и подтверждённым production provider-ом.
...
- работоспособность ASUS fallback является отдельным network gate, а не
  следствием наличия кода или локального WireGuard-профиля;
