...
На 2026-09-12 VM126 подтвердил `qwen3.8-flash-next` через отдельный приватный
маршрут KZ WireGuard hub `10.77.77.1:18002`; прямой ASUS-адрес наружу не
публикуется. GLM на `10.66.66.28:8000` остаётся третьим, но не считается
подтверждённым production fallback до отдельного успешного VM126 readback.
DeepSeek остаётся первым provider-ом.
