...
> **TL;DR**: Найдено **9 BUG** в production (5 CRITICAL, 4 medium):
>
> | # | Severity | Bug | Endpoint |
> |---|---|---|---|
> | 1 | 🔴 CRITICAL | RLS блокирует INSERT в `users` (register) | `POST /auth/register` → 500 |
> | 2 | 🔴 CRITICAL | То же для demo-login | `POST /auth/demo-login` → 500 |
> | 3 | 🔴 CRITICAL | SQLAlchemy 2.0 не имеет `Select.crossjoin` | `GET /admin/dashboard` → 500 |
> | 4 | 🔴 CRITICAL | Upstash Redis исчерпал 500K бесплатных requests | rate limiting DISABLED |
> | 5 | 🔴 CRITICAL | `tenant_settings = 0 rows` для всех 7 tenants | UI customization сломана |
...
**Этот ход (2026-07-10):** Askar указал — зачем локально, если есть прямой доступ к
живому production? Переключился на live QA через HTTPS к `kamilya-lms-api.onrender.com`
+ прямой SQL к Supabase pooler через `DATABASE_URL`. **5 минут vs 40.**
...
| DB | Supabase `postgres.ducegbxphkgffgozkchw` (eu-central-1 pooler) | DATABASE_URL |
...
## Live DB Snapshot (via DATABASE_URL → Supabase pooler)
...
quizzes                   100
certificates              0   -- CRITICAL: not a single cert issued
audit_logs                215
provider_keys             0   -- all keys from env, no DB redundancy
tenant_settings           0   -- CRITICAL: no customization for any tenant
```
...
Body: {"email":"qa-test-26344@kml-test.kz","first_name":"QA","last_name":"Tester","password":"QaTest123!"}
...
## Superadmin password reset (этот ход)

Пароль `admin@kamilya.kz` (platform-level superadmin, `tenant_id IS NULL`)
сменён на **`QA-Reset-2026-Xk9m`** через прямой `UPDATE users SET password_hash`
с argon2id hash. Argon2 password verification прошёл локально и через
`/auth/superadmin-login` → 200.

⚠️ **ВНИМАНИЕ:** Askar должен сменить этот пароль обратно после QA-сессии.
Пароль сейчас:
```
email:    admin@kamilya.kz
password: QA-Reset-2026-Xk9m
```
