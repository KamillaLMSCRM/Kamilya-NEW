...
> **Дата обновления:** 2026-07-01 (обновлено из старого `VPS_CONNECTION_GUIDE.md` от 24 июня)
...
| **Cache / Celery broker** | Upstash Redis | `REDIS_URL` в Render env |
...
| **Celery worker (apply-rules)** | **VPS** `173.249.51.164`, systemd `kamilya-worker.service` | забирает задачи из Upstash Redis очередь `celery` |
...
**То есть на этом VPS живут 4 prod-критичных сервиса:** `docling`, `wa-gateway`, `kamilya-worker`, WireGuard-туннель к DGX (через который LLM попадает в прод).

---

## Сервисы на VPS — реальное состояние на 2026-07-01
...
| `kamilya-worker.service` | — | — | **Celery worker** для фоновой задачи `apply_rules_for_users_task`. После `commit_import` в `/admin/staff` worker пересчитывает `Enrollment` строки (position→course mapping) асинхронно. Очередь: Upstash Redis, queue `celery`, pool `solo`, concurrency 1. Запущен 2026-06-30 на отдельном clone `/opt/kamilya-worker` (без `apps/web`), HEAD = `4b3f0d4`. | ✅ running, uptime 20h |
...
- ❌ **Postgres / Redis** — НЕ на VPS. DB = Supabase, Redis = Upstash. VPS только консумирует их по сети.
- ❌ **Celery broker** — НЕ на VPS. Worker подключается к Upstash Redis по `REDIS_URL` из `/opt/kamilya-worker/apps/api/.env`.
...
- Задача попадает в Upstash Redis queue `celery`.
...
Если видишь `ConnectionError: Error connecting to rediss://...` — Upstash недоступен (проверь `REDIS_URL` в `/opt/kamilya-worker/apps/api/.env`) или IP VPS не в Upstash allowlist (`dashboard.upstash.com → Networking → Allowed IP addresses`).
...
| `apply_rules_task_id` висит "pending" вечно | Worker упал или Upstash недоступен | `systemctl status kamilya-worker` + проверить Redis URL. Restart: `systemctl restart kamilya-worker`. |
