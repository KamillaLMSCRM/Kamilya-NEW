...
- Согласованный средний дневной бюджет: `$5`. Для обычного daily budget это
  ...
  `$70` 2026-08-07. Для запуска в понедельник запланирован рабочий интервал
  `2026-08-10–2026-08-23` (14 календарных дней).
- Расписание показа для первого теста: понедельник–пятница, `08:00–20:00` по
  часовому поясу аккаунта `GMT+05:00 (Казахстан)`. Это даёт два полных рабочих
  цикла, не расходует бюджет в выходные и захватывает поиск до и после обычного
  рабочего дня. Старт — 2026-08-10 в 08:00; последний фактический показ при
  таком расписании — 2026-08-21 до 20:00.
...
Состояние Google Ads Editor 2026-08-09: локально подготовлена конфигурация
`KZ | Finance | Search | RU`. Кампания paused; Search partners и Display
выключены; выбраны Казахстан с вариантом Presence и русский язык; даты
2026-08-10–2026-08-23; расписание понедельник–пятница 08:00–20:00
Asia/Almaty; campaign total budget `$70`; Maximize clicks с пределом CPC `$2`.
Импортированы и проверены 27 exact/phrase keywords, шесть групп, 10 paused RSA
и 20 campaign-level phrase negatives. AG1–AG4 и AG1b enabled только под paused
parent campaign, AG5 и его keywords paused. Кириллица побайтно сверена с
каноническими CSV. Кампания не опубликована в аккаунт, расход не начался.
...
Аудит готовности 2026-08-09: production landing `/ru/finance`, `/kk/finance`,
privacy, `robots.txt` и `sitemap.xml` отвечают `200`; локально прошли `11/11`
тестов, `pnpm typecheck` и production build. В Google Ads подтверждены вход в
аккаунт, часовой пояс `GMT+05:00 (Казахстан)`, настроенный способ оплаты,
нулевой баланс и отсутствие ожидающих платежей. Карточка конверсии показывает,
что action `Kamilya | Finance lead form` создана, но тег ещё не подтверждён.
Проверка через обычный Chrome ранее показала `Всего проектов: 0` и `0,00 $ в
день`: конфигурация мастера не была сохранена. Поэтому кампания собрана в
официальном Google Ads Editor 2.13.3 и оставлена локально неопубликованной.
Статус остаётся **NO-GO** до advertiser verification, независимого Tag Assistant
smoke и публикации в аккаунт только со статусом paused. После публикации нужно
ещё раз проверить настройки в web-кабинете перед включением parent campaign.
