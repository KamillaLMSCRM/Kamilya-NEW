...
- Согласованный средний дневной бюджет: `$5`. Для обычного daily budget это
  ...
  с заданными датами старта и завершения. Владелец подтвердил общий лимит
  `$70` 2026-08-07; в мастере выбраны даты 2026-08-08–2026-08-21.
- В мастере выбрана стратегия «Максимальное количество кликов» с пределом
  максимального CPC `$2`. Перед включением кампании предел нужно повторно
  проверить на итоговом экране аккаунта.
...
- [x] campaign total budget `$70` для 14-дневного теста подтверждён владельцем;
- [ ] кампания вручную переведена из paused только после всех проверок.

Состояние мастера Google Ads 2026-08-07: подготовлена конфигурация
`KZ | Finance | Search | RU`, Search partners и Display выключены, выбраны
Казахстан с вариантом Presence, русский язык, даты 2026-08-08–2026-08-21,
полный бюджет `$70`, шесть exact/phrase keywords и одно проверенное RSA.
Кампания не опубликована, расход не начался. Завершение заблокировано диалогом
Google «Подтверждение личности»: после ручного подтверждения необходимо
отключить AI Max, проверить отсутствие автогенерации текста и замены конечного
URL, добавить остальные группы/объявления из CSV и только затем сохранить
кампанию в paused. Текущую незавершённую конфигурацию нельзя считать
launch-ready.
