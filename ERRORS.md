...
## AI-002 — Retry теста потерял исходный урок и сгенерировал вопросы о JSON/HTTP

- Дата: 2026-08-20.
- Симптом: disposable production-курс успешно создался, но два первых теста
  спрашивали о JSON, HTTP и REST вместо материала загруженного регламента.
- Причина: после структурной ошибки retry-prompt заменял исходный урок текстом
  ошибки и фрагментом предыдущего ответа модели. Структурный validator проверял
  число вариантов, но не требовал доказательства связи вопроса с источником.
- Исправление: каждый MCQ теперь обязан содержать дословную опорную цитату из
  того же ограниченного фрагмента урока, вопрос использует минимум два её
  термина, а правильный вариант является дословным фрагментом цитаты. Retry
  заново включает неизменный урок и не включает сырой прошлый ответ; lesson data
  помечены недоверенными, source title принадлежит серверу, raw response не
  журналируется.
- Проверка: unit-suite `261 passed`; отдельные тесты отклоняют структурно
  корректный REST/JSON-ответ даже с настоящей, но неиспользованной цитатой,
  проверяют пять исчерпанных retry, 8000-символьную границу, короткий источник,
  подмену title и сохранение source quote. Production regeneration выполняется
  отдельной строкой acceptance после выпуска exact commit.
