...
- Исправление: retry каждый раз строится из неизменного урока без сырого
  ...
  принадлежит серверу, response body не журналируется.
- Проверка: unit-suite `262 passed`; 37 focused assessment/failover/release
  tests прошли. Candidate-код выполнен внутри production AI-worker на
  синтетическом уроке: 5 вопросов, 5 серверных цитат, 0 неподдержанных
  мета-терминов, 92.98 секунды. Полный production job повторяется после выпуска
  exact commit; прямой Qwen 3.8 endpoint с VM126 пока недоступен, поэтому его
  free-pool нельзя включать до отдельного сетевого gate.
